...
const BREAKDOWN_STAGE_META = [
    ...
    'TESTING' => ['department' => 'Technician', 'slaHours' => 8],
...
function bw_technician_workload(array $rows): array {
    ...
    $jobCounts = db()->prepare("SELECT
        ...
        SUM(CASE WHEN status IN ('RECEIVED','IN_PROGRESS','TESTING') THEN 1 ELSE 0 END) AS in_progress,
...
// V416 - machine-specific operational analysis for the Machine Report Center.
// The scope is deliberately ONE machine; no fleet/customer totals are mixed in.
if ($method === 'GET' && $action === 'machine-analysis-pdf' && !empty($_GET['machineId'])) {
    if (bw_is_technician_actor($ctx)) json_error('Technicians can download only their assigned Job Card/Report PDFs.',403);
    $machineId = trim((string)$_GET['machineId']);
    $machine = bw_machine_report_access($ctx, $machineId);
    [$fromTs, $toTs, $periodLabel] = bw_machine_report_range((string)($_GET['from'] ?? ''), (string)($_GET['to'] ?? ''));

    $checkSql = 'SELECT overall_status,hour_meter_reading,created_at FROM checklist_reports WHERE machine_id=?';
    $checkParams = [$machineId];
    if ($fromTs !== null) { $checkSql .= ' AND created_at >= ?'; $checkParams[] = $fromTs; }
    if ($toTs !== null) { $checkSql .= ' AND created_at < ?'; $checkParams[] = $toTs; }
    $checkSql .= ' ORDER BY created_at DESC';
    $stmt = db()->prepare($checkSql); $stmt->execute($checkParams); $checks = $stmt->fetchAll();

    $jobSql = 'SELECT title,fault_description,status,repeat_issue,started_at,completed_at,created_at FROM digital_job_cards WHERE machine_id=?';
    $jobParams = [$machineId];
    if ($fromTs !== null) { $jobSql .= ' AND created_at >= ?'; $jobParams[] = $fromTs; }
    if ($toTs !== null) { $jobSql .= ' AND created_at < ?'; $jobParams[] = $toTs; }
    $jobSql .= ' ORDER BY created_at DESC';
    $stmt = db()->prepare($jobSql); $stmt->execute($jobParams); $jobs = $stmt->fetchAll();

    $opSql = 'SELECT status,created_at FROM operator_reports WHERE machine_id=?';
    $opParams = [$machineId];
    if ($fromTs !== null) { $opSql .= ' AND created_at >= ?'; $opParams[] = $fromTs; }
    if ($toTs !== null) { $opSql .= ' AND created_at < ?'; $opParams[] = $toTs; }
    $opSql .= ' ORDER BY created_at DESC';
    $stmt = db()->prepare($opSql); $stmt->execute($opParams); $operatorReports = $stmt->fetchAll();

    $completed = array_values(array_filter($jobs, static fn(array $j): bool => strtoupper((string)$j['status']) === 'COMPLETED'));
    $openJobs = array_values(array_filter($jobs, static fn(array $j): bool => !in_array(strtoupper((string)$j['status']), ['COMPLETED','CANCELLED'], true)));
    $repeatJobs = array_values(array_filter($completed, static fn(array $j): bool => !empty($j['repeat_issue'])));
    $firstFixCount = max(0, count($completed) - count($repeatJobs));
    $firstFixRate = count($completed) ? (int)round(($firstFixCount / count($completed)) * 100) : 0;
    $completionRate = count($jobs) ? (int)round((count($completed) / count($jobs)) * 100) : 0;

    $redChecks = 0; $yellowChecks = 0; $hours = [];
    foreach ($checks as $check) {
        $st = strtoupper((string)($check['overall_status'] ?? ''));
        if (in_array($st, ['RED','CRITICAL'], true)) $redChecks++;
        if (in_array($st, ['YELLOW','ATTENTION'], true)) $yellowChecks++;
        if ($check['hour_meter_reading'] !== null && is_numeric($check['hour_meter_reading'])) $hours[] = (float)$check['hour_meter_reading'];
    }
    $latestHours = count($hours) ? $hours[0] : null;
    $oldestHours = count($hours) ? $hours[count($hours)-1] : null;
    $hoursMovement = ($latestHours !== null && $oldestHours !== null) ? max(0, $latestHours - $oldestHours) : null;
    $latestCondition = count($checks) ? strtoupper((string)($checks[0]['overall_status'] ?: 'GREEN')) : 'NOT CHECKED';

    $resolutionHours = [];
    foreach ($completed as $job) {
        $startRaw = $job['started_at'] ?: $job['created_at'];
        $endRaw = $job['completed_at'] ?: null;
        if (!$startRaw || !$endRaw) continue;
        try {
            $start = new DateTimeImmutable((string)$startRaw);
            $end = new DateTimeImmutable((string)$endRaw);
            if ($end >= $start) $resolutionHours[] = ($end->getTimestamp() - $start->getTimestamp()) / 3600;
        } catch (Throwable $ignored) {}
    }
    $avgRepair = count($resolutionHours) ? array_sum($resolutionHours) / count($resolutionHours) : 0;

    $openOperator = 0; $resolvedOperator = 0;
    foreach ($operatorReports as $report) {
        if (strtoupper((string)$report['status']) === 'RESOLVED') $resolvedOperator++; else $openOperator++;
    }
    $operatorResolutionRate = count($operatorReports) ? (int)round(($resolvedOperator / count($operatorReports)) * 100) : 0;

    $faults = [];
    foreach ($jobs as $job) {
        $name = trim((string)($job['fault_description'] ?: $job['title'] ?: 'Unspecified job'));
        if ($name === '') $name = 'Unspecified job';
        if (!isset($faults[$name])) $faults[$name] = ['count'=>0,'repeat'=>0];
        $faults[$name]['count']++;
        if (!empty($job['repeat_issue'])) $faults[$name]['repeat']++;
    }
    uasort($faults, static function(array $a, array $b): int {
        if ($a['count'] === $b['count']) return $b['repeat'] <=> $a['repeat'];
        return $b['count'] <=> $a['count'];
    });

    $rows = [
        ['MACHINE HEALTH'],
        ['Latest condition', $latestCondition],
        ['Checklist reports', (string)count($checks)],
        ["Red / Don't operate", (string)$redChecks],
        ['Yellow / Attention', (string)$yellowChecks],
        ['Latest hour meter', $latestHours === null ? '-' : (string)$latestHours],
        ['Hours movement in period', $hoursMovement === null ? '-' : round($hoursMovement,1).' HRS'],
        [''],
        ['JOB CARD PERFORMANCE'],
        ['Job Cards', (string)count($jobs)],
        ['Open Job Cards', (string)count($openJobs)],
        ['Completed', (string)count($completed)],
        ['Completion rate', $completionRate.'%'],
        ['First-time fix', $firstFixRate.'%'],
        ['Repeat / rework', (string)count($repeatJobs)],
        ['Average repair time', round($avgRepair,1).' hrs'],
        [''],
        ['OPERATOR REPORTS'],
        ['Total reports', (string)count($operatorReports)],
        ['Open', (string)$openOperator],
        ['Resolved', (string)$resolvedOperator],
        ['Resolution rate', $operatorResolutionRate.'%'],
        [''],
        ['COMMON FAULTS / JOBS'],
        ['Fault / Job','Occurrences','Repeat / rework'],
    ];
    $i=0;
    foreach ($faults as $name => $info) {
        $rows[] = [$name,(string)$info['count'],(string)$info['repeat']];
        if (++$i >= 10) break;
    }
    if ($i===0) $rows[]=['No Job Card fault history in this period','0','0'];

    $machineLabel = trim((string)($machine['brand'] ?? '') . ' ' . (string)($machine['model'] ?? '')) ?: 'Machine';
    $safeMachine = preg_replace('/[^A-Za-z0-9_-]+/', '-', $machineLabel);
    output_table_pdf(
        'BELM-machine-analysis-' . $safeMachine . '.pdf',
        'MACHINE ANALYSIS',
        [
            'Customer: ' . (string)($machine['customer_name'] ?? 'Customer'),
            'Machine: ' . $machineLabel,
...
if ($method === 'GET' && $action === 'job-card-pdf' && $id !== '') {
    ...
    $rows=[
        ...
        ['Received / Opened by Technician', display_date_billing($job['started_at'])],
        ['Testing', strtoupper((string)$job['status'])==='TESTING' ? 'IN PROGRESS' : ($job['test_result'] ? 'RECORDED' : '-')],
...
if ($method === 'GET' && $action === 'case' && $id !== '') {
    ...
    foreach($jobRows as &$jobRow){
        ...
        if ($jobTechId==='' && $sourceTechnicianId!=='' && in_array($jobState,['ASSIGNED','RECEIVED','IN_PROGRESS','TESTING'],true)) {
...
if ($method === 'POST' && $action === 'job-card') {
    ...
    if($ctx['kind']==='customer' && $jobCardMode==='existing') {
        ...
        $preservedStatus=$reassigned?'ASSIGNED':(in_array($currentStatus,['RECEIVED','IN_PROGRESS','WAITING_FOR_PARTS','TESTING'],true)?$currentStatus:'ASSIGNED');
    ...
    if ($existingJobRow) {
        ...
        if (!empty($existingJobRow['technician_id'] ?? null) || in_array(strtoupper((string)$existingJobRow['status']), ['ASSIGNED', 'RECEIVED', 'IN_PROGRESS', 'WAITING_FOR_PARTS', 'TESTING', 'PENDING_APPROVAL'], true)) {
...
// V447 - opening an assigned Job Card means RECEIVED, not work already in progress.
// started_at is retained as the auditable first-open/received timestamp. Repeat opens are idempotent.
if ($method === 'PUT' && $action === 'job-open' && $id !== '') {
    ...
    bw_require_assigned_job($ctx,$job);
    $currentStatus=strtoupper(trim((string)($job['status']??'')));
    if(in_array($currentStatus,['PENDING_APPROVAL','COMPLETED','CANCELLED'],true)) {
        json_error('This Job Card is already submitted or closed.',409);
    }
    $firstOpen=empty($job['started_at']) || $currentStatus==='ASSIGNED';
    $newStatus=in_array($currentStatus,['ASSIGNED','OPEN','CREATED'],true)?'RECEIVED':($currentStatus?:'RECEIVED');
    if($firstOpen || $newStatus!==$currentStatus){
        db()->prepare("UPDATE digital_job_cards SET started_at=COALESCE(started_at,NOW()),status=?,updated_at=NOW() WHERE id=?")
            ->execute([$newStatus,$id]);
        if($firstOpen){
            bw_log((string)$job['case_id'],'JOB_CARD_RECEIVED','Technician','Job Card received by Technician',null,$ctx);
        }
    }
    json_out(['ok'=>true,'firstOpen'=>$firstOpen,'process'=>'RECEIVED','status'=>$newStatus]);
}
...
if ($method === 'PUT' && $action === 'job-report' && $id !== '') {
    ...
    $testResult=trim((string)($b['testResult']??''));
    ...
    if($complete && $work==='') json_error('Work done is required before submitting the Job Card for Manager Review.');
    if($complete && $testResult==='') json_error('Test Result is required before the Technician can complete this Job Card.');
    if($complete && $currentJobStatus!=='TESTING') json_error('Save the Test Result first to move this Job Card into TESTING. Then reopen it and submit the completed work for Manager Review.',409);
    ...
    $nextJobStatus=($requiredSpare!==''||$activeSpareCount>0)?'WAITING_FOR_PARTS':($complete?'PENDING_APPROVAL':(($testResult!==''&&$work!=='')?'TESTING':'IN_PROGRESS'));
    db()->prepare("UPDATE digital_job_cards SET technician_id=?,technician_name=?,diagnosis=?,work_done=?,test_result=?,completion_note=?,repeat_issue=?,status=?,started_at=COALESCE(started_at,NOW()),technician_submitted_at=CASE WHEN ? THEN NOW() ELSE technician_submitted_at END,completed_at=NULL,reviewed_at=CASE WHEN ? THEN NULL ELSE reviewed_at END,reviewed_by_name=CASE WHEN ? THEN NULL ELSE reviewed_by_name END,review_note=CASE WHEN ? THEN NULL ELSE review_note END,updated_at=NOW() WHERE id=?")
        ->execute([$ctx['actorId'],$ctx['actorName'],$diagnosis,$work?:null,$testResult?:null,trim((string)($b['completionNote']??''))?:null,$repeat?1:0,$nextJobStatus,$complete?1:0,$complete?1:0,$complete?1:0,$complete?1:0,$id]);
    $spareCreated=false;
    if($requiredSpare!==''){
...
Open Breakdown Workflow to approve or reject.");
        ...
        if($complete){
            bw_set_stage($job['case_id'],'PENDING_APPROVAL',null,$ctx,'Technician completed Job Card - waiting Manager Review');
        }elseif($nextJobStatus==='TESTING'){
            bw_set_stage($job['case_id'],'TESTING',null,$ctx,'Repair completed - Technician testing in progress');
        }else{
            bw_set_stage($job['case_id'],'REPAIR',null,$ctx,'Technician Diagnosis / Repair progress saved');
        }
    }
    ...
    $jobReportStatus=($requiredSpare!==''||$activeSpareCount>0)?'Waiting for Parts':($complete?'Completed by Technician - Manager Review':($nextJobStatus==='TESTING'?'Testing':'Repair in Progress'));
