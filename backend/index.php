...
if (($segments[0] ?? '') === 'readiness' || !isset($segments[0])) {
    try {
        ...
        $requiredTables = [
            'roles','users','customers','customer_users','machines','customer_applications','user_applications','usage_logs',
            'customer_store_items','customer_store_movements','customer_machine_spare_list_items','customer_store_issue_requests',
            'belm_workshop_tool_issues','delivery_notes','delivery_note_items','customer_procurement_requests','customer_department_settings',
            'customer_sales_documents','checklist_template_parts','service_request_parts','spare_parts','spare_part_requests','bank_accounts',
            'bank_withdrawals','company_expenses','customer_communications','notification_logs','system_settings','machine_service_owner_notifications',
            'user_preferences','machine_service_parts','service_due_alerts','service_due_alert_items','breakdown_cases','breakdown_case_events',
            'breakdown_spare_requests','digital_job_cards','invoices','invoice_items','payments','receipts','proforma_invoices','proforma_invoice_items',
            'belm_installation_meta','belm_schema_migrations','belm_deployment_audits',
        ];
        $tableChecks = [];
        $schemaReady = true;
        ...
        $requiredColumns = [
            ['digital_job_cards','issued_by_name'],['digital_job_cards','signed_copy_data'],['digital_job_cards','billing_status'],
            ['digital_job_cards','priority'],['digital_job_cards','due_date'],['invoices','source_job_card_id'],['proforma_invoices','source_job_card_id'],
            ['password_reset_codes','account_id'],['payments','receipt_id'],['bank_accounts','is_test'],['spare_part_requests','procurement_order_status'],
            ['breakdown_spare_requests','procurement_supplier_id'],['customers','coordinator_features'],['customers','is_machinery_admin'],
            ['customer_department_settings','department_key'],['customer_department_settings','access_state'],['customer_sales_documents','document_type'],
            ['customer_sales_documents','document_no'],
        ];
        $columnChecks = [];
        $columnStatement = db()->prepare("SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=? AND column_name=?)");
        ...
        try {
            $stmt = db()->prepare("SELECT u.id,u.is_active,u.deleted_at,u.password_hash,r.name AS role_name,COUNT(*) OVER () AS matching_accounts FROM users u LEFT JOIN roles r ON r.id=u.role_id WHERE u.id=? ORDER BY CASE WHEN u.deleted_at IS NULL AND u.is_active=1 THEN 0 ELSE 1 END,u.created_at ASC LIMIT 1");
            $stmt->execute(['00000000-0000-4000-8000-000000000003']);
            $admin = $stmt->fetch();
            if ($admin) {
                $hash = (string)($admin['password_hash'] ?? '');
                $adminChecks['exactlyOneAccount'] = (int)$admin['matching_accounts'] === 1;
                $adminChecks['active'] = (int)$admin['is_active'] === 1 && $admin['deleted_at'] === null;
                $adminChecks['superAdminRole'] = $admin['role_name'] === 'Super Admin';
                $adminChecks['passwordHashStored'] = str_starts_with($hash,'$2') || str_starts_with($hash,'$argon2');
            }
        } catch (Throwable $ignored) {}
        $adminReady = !in_array(false,$adminChecks,true);
        $dataSafety = ['storage'=>'PostgreSQL','installationId'=>null,'lastDeploymentRelease'=>null,'lastDeploymentAt'=>null,'fullResetProtected'=>strtolower((string)(getenv('APP_ENV') ?: '')) === 'production' && trim((string)(getenv('ALLOW_FULL_DATABASE_RESET') ?: '')) !== 'YES-I-UNDERSTAND','webStartupIndependentOfMigration'=>true];
        try {
...
switch ($resource) {
    ...
    case 'customer_checkup.php':
        dispatch('customer_checkup.php');
    ...
    case 'checklist-reports': dispatch('checklist_reports.php',['action'=>'submit']);
