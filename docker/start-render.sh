...
run_safe_migration() {
    ...
    while :; do
        ...
        if [ "$code" -eq 0 ]; then
            ...
            php /var/www/html/api/scripts/settings_integrity.php || echo "BELM settings integrity check blocked a settings change; no duplicate setting rows were deleted." >&2
