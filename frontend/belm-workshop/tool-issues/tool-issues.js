(function(){
  ...
  function render(){
    ...
    body.innerHTML=visible.length?visible.map(x=>{
      ...
      return `<tr><td><b>${esc(x.documentNo)}</b></td><td>${esc(x.jobCardNo||'—')}</td><td>${esc(x.technicianName||'—')}</td><td><b>${esc(x.toolName)}</b>${x.toolAssetId?`<br><small>${esc(x.toolAssetId)}</small>`:''}</td><td>${esc(x.quantity)}</td><td>${fmt(x.issuedAt)}<br><small>${esc(x.issuedByName||'')}</small></td><td>${fmt(x.expectedReturnAt)}</td><td><span class="status-pill ${x.returnedAt?'returned':overdue?'overdue':''}">${status}</span></td><td>${x.returnedAt?`${fmt(x.returnedAt)}<br><small>${esc(x.receivedBy||'')}</small>`:`<button class="return-button" data-return="${esc(x.id)}">Receive Return</button>`}</td></tr>`;
    }).join(''):'<tr><td colspan="9" class="empty">No Tool Issue Documents found.</td></tr>';
  ...
  async function load(){
    ...
    }catch(e){show(e.message,true);document.getElementById('issueRows').innerHTML=`<tr><td colspan="9" class="empty">${esc(e.message)}</td></tr>`}
  }
