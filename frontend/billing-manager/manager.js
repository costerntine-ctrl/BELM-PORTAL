  ...
  function renderInvoices() {
    ...
    panel.innerHTML = `${reviewHeading("Invoices", "Review invoice totals, balances, due dates and status.", `/api/billing?action=export-invoices&token=${encodeURIComponent(token)}`)}<div class="table-wrap"><table><thead><tr><th>Invoice</th><th>Customer</th><th>Total</th><th>Paid</th><th>Balance</th><th>Due</th><th>Status</th><th></th></tr></thead><tbody>${invoices.map((invoice) => `
      <tr>
        ...
        <td><div class="row-actions"><button class="edit" data-edit-invoice="${escapeHtml(invoice.id)}">Re-edit</button><a class="export-row-button" href="/api/billing?action=export-invoice&id=${escapeHtml(invoice.id)}&token=${encodeURIComponent(token)}" target="_blank" rel="noopener">Export</a>${Number(invoice.balance) > 0 && invoice.status !== "CANCELLED" ? `<button class="pay" data-payment="${escapeHtml(invoice.id)}">Add payment</button>` : ""}<button class="delete" data-delete-invoice="${escapeHtml(invoice.id)}">Delete</button></div></td>
