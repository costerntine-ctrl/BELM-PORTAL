(function(){
  ...
  function processStageDetailHtml(stageKey,detail,c,mainJob){
    ...
    const technicianReceivedEvent=latestProcessEvent(detail,e=>String(e?.stage||'').toUpperCase()==='JOB_CARD_RECEIVED'||/job card received by technician/.test(eventText(e)));
    ...
    if(key==='TECH_RECEIVED'){
      return `<div class="stage-detail-head"><div><span>STAGE 3</span><h4>Received by Technician</h4></div><span class="pill green">${['RECEIVED','IN_PROGRESS','WAITING_FOR_PARTS','TESTING','PENDING_APPROVAL','COMPLETED'].includes(jobStatus)?'RECEIVED':'WAITING'}</span></div><div class="stage-detail-grid">${info('Technician',esc(techName||'Unassigned'))}${info('Received / opened at',fmtDate(technicianReceivedEvent?.created_at||mainJob.started_at))}${info('Current status',esc(jobStatus||'ASSIGNED'))}${info('Next action',jobStatus==='RECEIVED'?'Diagnosis / start repair':'See current process stage')}</div>${note(technicianReceivedEvent)}`;
    }
    if(key==='IN_PROGRESS'){
      ...
      return `<div class="stage-detail-head"><div><span>STAGE 4</span><h4>${jobStatus==='WAITING_FOR_PARTS'?'Waiting for Parts':'Diagnosis / Repair in Progress'}</h4></div><span class="pill">${esc(jobStatus||'WAITING')}</span></div><div class="stage-detail-grid">${info('Technician',esc(techName||'Unassigned'))}${info('Started',fmtDate(mainJob.started_at||last?.created_at))}${info('Diagnosis',esc(mainJob.diagnosis||'Not reported yet'))}${info('Work done',esc(mainJob.work_done||'No progress report yet'))}</div>${recent||'<div class="stage-detail-empty">No Technician progress update recorded yet.</div>'}`;
    }
    if(key==='SPARES'){
    ...
    if(key==='TESTING'){
      return `<div class="stage-detail-head"><div><span>STAGE 5</span><h4>Testing</h4></div><span class="pill yellow">${jobStatus==='TESTING'?'TESTING':mainJob.test_result?'RECORDED':'WAITING'}</span></div><div class="stage-detail-grid">${info('Technician',esc(techName||'Unassigned'))}${info('Testing started',fmtDate(testingEvent?.created_at))}${info('Test result',esc(mainJob.test_result||'Not recorded yet'))}${info('Completion note',esc(mainJob.completion_note||'-'))}</div>${note(testingEvent)}`;
    }
    if(key==='PENDING_APPROVAL'){
      const pending=jobStatus==='PENDING_APPROVAL';
      return `<div class="stage-detail-head"><div><span>STAGE 6</span><h4>Completed by Technician / Manager Review</h4></div><span class="pill yellow">${pending?'WAITING MANAGER REVIEW':'WAITING'}</span></div><div class="stage-detail-grid">${info('Technician',esc(techName||'Unassigned'))}${info('Technician completed',fmtDate(mainJob.technician_submitted_at))}${info('Test result',esc(mainJob.test_result||'Not recorded'))}${info('Completion note',esc(mainJob.completion_note||'-'))}</div>${mainJob.review_note?`<div class="stage-detail-event"><b>Latest review</b><p>${esc(mainJob.review_note)}</p><small>${esc(mainJob.reviewed_by_name||'Reviewer')} · ${fmtDate(mainJob.reviewed_at)}</small></div>`:''}`;
    }
    if(key==='COMPLETED'){
      return `<div class="stage-detail-head"><div><span>STAGE 7</span><h4>Closed / Returned to Service</h4></div><span class="pill green">${String(c.status||'').toUpperCase()==='COMPLETED'?'COMPLETED':'WAITING'}</span></div><div class="stage-detail-grid">${info('Technician',esc(techName||'Unassigned'))}${info('Completed at',fmtDate(mainJob.completed_at||completedEvent?.created_at))}${info('Closed by',esc(completedEvent?.actor_name||'-'))}${info('Signed Job Card',mainJob.has_signed_copy?'Yes':'Not uploaded')}</div>${note(completedEvent)}`;
    }
  ...
  function renderDetail(){
    ...
    const processKey=c.status==='COMPLETED'||jobStatus==='COMPLETED'?'COMPLETED':
      ...
      (jobStatus==='TESTING'||c.stage==='TESTING'?'TESTING':
      (jobStatus==='IN_PROGRESS'||waitingParts||['DIAGNOSIS','REPAIR'].includes(c.stage)?'IN_PROGRESS':
      (jobStatus==='RECEIVED'?'TECH_RECEIVED':
    ...
    const steps=[['RECEIVED',receivedStepLabel],['ASSIGNED','Assigned'],['TECH_RECEIVED','Technician Received'],['IN_PROGRESS',waitingParts?'Waiting for Parts':'Diagnosis / Repair'],['TESTING','Testing'],['PENDING_APPROVAL','Manager Review'],['COMPLETED','Closed']];
  ...
  function wireDetail(){
    ...
    document.querySelectorAll('[data-tech-report]').forEach(b=>b.onclick=async()=>{try{await api(`/job-open/${encodeURIComponent(b.dataset.techReport)}`,{method:'PUT',body:JSON.stringify({})});document.getElementById('techJobId').value=b.dataset.techReport;document.getElementById('techReportDialog').showModal();await openCase(selected.case.id)}catch(x){show(x.message,true)}});
  ...
  document.getElementById('techReportForm').onsubmit=async e=>{e.preventDefault();try{await api(`/job-report/${document.getElementById('techJobId').value}`,{method:'PUT',body:JSON.stringify({diagnosis:document.getElementById('techDiagnosis').value,workDone:document.getElementById('techWork').value,testResult:document.getElementById('techTest').value,completionNote:document.getElementById('techNote').value,repeatIssue:document.getElementById('techRepeat').checked,complete:document.getElementById('techComplete').checked})});document.getElementById('techReportDialog').close();show(document.getElementById('techComplete').checked?'Technician work completed and sent to Manager Review.':(document.getElementById('techTest').value.trim()?'Testing stage saved.':'Digital Job Card repair progress saved.'));await load();await openCase(selected.case.id)}catch(x){show(x.message,true)}};
