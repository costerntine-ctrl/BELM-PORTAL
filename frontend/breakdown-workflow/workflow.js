(function(){
  ...
  if(isBelmAdmin&&!embedded){
    const target=new URL('/belm-workshop/',location.origin);
  ...
  document.getElementById('backButton').onclick=()=>{if(embedded){window.parent.postMessage({type:'belm-engineering-open-service-requests'},window.location.origin);return;}location.href=source==='customer'?'/portal/dashboard':source==='tech'?'/tech':'/belm-workshop/#job-cards'};
