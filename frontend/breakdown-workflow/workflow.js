(function(){
  ...
  function renderDetail(){
    ...
    document.getElementById('caseDetail').innerHTML=`<div class="detail">
      ...
      <section class="workflow-tab-panel" data-workflow-panel="jobs"><div class="section tab-section"><h3>Digital Job Cards</h3>${jobsHtml}</div></section>
      ...
      <section class="workflow-tab-panel" data-workflow-panel="timeline"><div class="section tab-section"><h3>Process Timeline</h3>${timelineHtml}</div></section>
    </div>`;
    wireDetail();
  }
  ...
  async function load(){try{cases=await api(machineFilter?`?machineId=${encodeURIComponent(machineFilter)}`:'');renderSummary();renderList();loadPerformance();loadDepartmentReport()}catch(x){show(x.message,true);document.getElementById('caseList').innerHTML=`<div class="empty">${esc(x.message)}</div>`}}
  loadMachines();load();
})();
