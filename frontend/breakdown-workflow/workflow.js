(function(){
  ...
  if(isBelmAdmin&&!embedded){
    const target=new URL('/concept-dashboards/11-workshop-manager/',location.origin);
  ...
  function departmentLabel(value){return String(value||'').replace(/Administration \/ Engineering/gi,'Administration / Technical Dep').replace(/\bEngineering\b/gi,'Technical Dep')}
  ...
  document.getElementById('backButton').onclick=()=>{if(embedded){window.parent.postMessage({type:'belm-engineering-open-service-requests'},window.location.origin);return;}location.href=source==='customer'?'/portal/dashboard':source==='tech'?'/tech':'/concept-dashboards/11-workshop-manager/job-cards.html'};
  ...
  function renderList(){
    ...
    document.getElementById('caseList').innerHTML=list.length?list.map(c=>{
      ...
      return `<article class="case-card attention ${c.delayed?'delayed':''}" data-case="${esc(c.id)}">
        ...
        <div class="case-meta"><span class="pill status-pill ${c.delayed?'red':'yellow'}">${esc(c.delayed?'DELAYED':c.status)}</span><span class="pill department-pill ${departmentColorClass(c)}">${esc(departmentLabel(c.department))}</span><span class="pill breakdown-time-pill">Breakdown ${esc(duration(c.breakdownHours))}</span>${c.delayed?`<span class="pill red delay-time-pill">Delay ${esc(duration(c.delayHours))}</span>`:''}</div>
        ${assignment}
      </article>`;
  ...
  function renderDetail(){
    ...
    const focusTitle=c.status==='COMPLETED'?'COMPLETED - MACHINE RETURNED TO SERVICE':awaitingAssignment?`${c.delayed?'DELAYED - ':''}AWAITING TECHNICIAN ASSIGNMENT - WORKSHOP / DISPATCH OWNS THE NEXT ACTION`:`${c.delayed?'DELAYED - ':''}${esc(departmentLabel(c.department))} OWNS THE NEXT ACTION`;
    ...
    const timelineHtml=d.events.length?`<div class="timeline">${d.events.map(e=>`<div class="event"><b>${esc(e.action)} - ${esc(departmentLabel(e.department))}</b><div>${esc(e.note||'')}</div><small>${esc(e.actor_name||'System')} · ${fmtDate(e.created_at)}</small></div>`).join('')}</div>`:'<div class="empty">No process event recorded.</div>';
    ...
    const mainJobStatusClass=jobStatus==='RECEIVED'?'green':jobStatus==='COMPLETED'?'green':jobStatus==='ASSIGNED'?'blue':jobStatus==='IN_PROGRESS'?'yellow':jobStatus==='PENDING_APPROVAL'?'yellow':'';
    ...
    document.getElementById('caseDetail').innerHTML=`<div class="detail">
      ...
      <div class="status-box"><div><span>Breakdown Time</span><b>${esc(duration(c.breakdownHours))}</b></div><div><span>Current Stage</span><b>${esc(c.stage.replaceAll('_',' '))}</b></div><div><span>Department</span><b>${esc(departmentLabel(c.department))}</b></div><div><span>Waiting Here</span><b>${esc(duration(c.stageHours))}</b></div></div>
      ${processHtml}
      <div class="actions workflow-primary-actions">${isWorkshop&&c.status!=='COMPLETED'&&(source!=='customer'||(c.customerManagesWorkshop&&!officialBelmJob))&&(!mainJob||!['PENDING_APPROVAL','COMPLETED','CANCELLED'].includes(jobStatus))?`<button class="${source==='customer'&&mainJob?'yellow':'red'}" id="generateJob">${source==='customer'&&mainJob?'Manage / Reassign Job Card':'Technician Job Card Assignment'}</button>`:''}${source==='customer'&&isCustomerAdmin&&c.status!=='COMPLETED'&&d.jobCards.length===0&&!c.customerManagesWorkshop?'<button class="blue" id="sendJobToBelm">Send Job Card to BELM</button>':''}</div>
