...
(function(){
  ...
  function card(m,latest){const s=machineStatus(m),id=val(m,'id'),label=val(m,'label')||[val(m,'brand'),val(m,'model')].filter(Boolean).join(' ')||val(m,'machineType','machine_type')||'Machine',fleet=val(m,'fleetNumber','fleet_number')||'—',serial=val(m,'serialNumber','serial_number')||'—',reg=val(m,'regNumber','reg_number')||'—';const checked=latest&&eatDay(latest.createdAt||latest.created_at||latest.submittedAt||latest.submitted_at)===today();const state=checked?`<div class="daily-state checked">✓ Checked today · ${esc(latest.templateName||latest.template_name||'Checklist')} · ${esc(latest.status||'GREEN')} · Hrs ${esc(latest.hourMeterReading??latest.hour_meter_reading??'—')}</div>`:`<div class="daily-state attention">! Daily check pending for today</div>`;return `<article class="assigned-machine-card status-${s[0]}" data-machine-id="${esc(id)}"><div class="machine-card-top"><div><small>DAILY CHECKLIST MACHINE</small><h3>${esc(label)}</h3><p>Fleet ${esc(fleet)}</p></div><span class="machine-status-pill">${esc(s[1])}</span></div><div class="machine-card-info"><div><span>Fleet No.</span><b>${esc(fleet)}</b></div><div><span>Serial No.</span><b>${esc(serial)}</b></div><div><span>Registration</span><b>${esc(reg)}</b></div></div>${state}<div class="machine-actions"><a class="gold" href="/concept-dashboards/08-daily-checklist/?actor=technician&machine=${encodeURIComponent(id)}">Open Daily Checklist</a><a href="/tech-report/?machineId=${encodeURIComponent(id)}&category=checklists">Checklist Reports</a><a href="/technician-job-cards/?machine=${encodeURIComponent(id)}">Job Cards</a></div></article>`}
