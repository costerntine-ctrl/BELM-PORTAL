document.addEventListener('DOMContentLoaded', function () {
  ...
  fetchJson('/api/readiness').then(function(data){setInfo('Database',data&&data.database==='connected'?'<span class="dot-live"></span>PostgreSQL (Connected)':'Database not ready');}).catch(function(){setInfo('Database','Connection check failed');});

  var buttons=[...document.querySelectorAll('.quick-actions-grid .qa-btn')];
  buttons.forEach(function(b){
    var label=b.textContent.replace(/\s+/g,' ').trim().toLowerCase();
    if(label.includes('clear cache')) b.addEventListener('click',async function(){b.disabled=true;try{if('caches' in window){var keys=await caches.keys();await Promise.all(keys.filter(function(k){return k.indexOf('belm-portal')===0;}).map(function(k){return caches.delete(k);}));}if(navigator.serviceWorker&&navigator.serviceWorker.controller)navigator.serviceWorker.controller.postMessage({type:'CLEAR_BELM_CACHES'});toast('Portal cache cleared. Reloading fresh files…');setTimeout(function(){location.reload();},700);}catch(e){toast(e.message,true);b.disabled=false;}});
    ...
    else if(label.includes('restart services')) b.addEventListener('click',async function(){b.disabled=true;try{await fetchJson('/api/readiness');toast('Application and database readiness checked successfully.');setTimeout(function(){location.reload();},700);}catch(e){toast('Service check failed: '+e.message,true);}finally{setTimeout(function(){b.disabled=false;},900);}});
    else if(label.includes('maintenance mode')) b.addEventListener('click',function(){location.href='/settings-manager/?section=maintenance&module=settings';});
  });
