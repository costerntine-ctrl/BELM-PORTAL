(()=>{
  ...
  if(role!=='super admin'&&user?.allowedPages!==null){location.replace('/concept-dashboards/11-workshop-manager/');return}
  let all=[];
  ...
  function currentPayload(c){const f=c.coordinatorFeatures||{};return{invoiceSystem:true,proformaSystem:true,operatorDashboard:f.operatorDashboard!==false,technicianDashboard:f.technicianDashboard!==false,machineCardButtons:{operator:{report:buttonState(f,'report','enabled'),checkup:buttonState(f,'checkup','enabled'),parts:buttonState(f,'parts','disabled'),operationCard:buttonState(f,'operationCard','enabled')}}}}
  function wire(){
