(function () {
  ...
  function render(data) {
    ...
    document.getElementById("pageTitle").textContent = `Job Card — ${machineLabel}`;
    const notice = document.getElementById("supportModeNotice");
    notice.classList.remove("hidden", "error");
    if (selfServiceMode) {
      notice.textContent = `SELF-SERVICE MODE: Your own technicians/operators handle normal maintenance. Use this Job Card only when you want BELM involved. Customer Admin can send it to ${belmBusiness.email || "BELM Business Email"}.`;
      document.getElementById("portalModeSubtitle").textContent = "Customer Self-Service · Job Card Gateway";
      document.getElementById("servicePanelTitle").textContent = "BELM Job Card details";
      document.getElementById("servicePanelIntro").textContent = "Describe the work required. Customer Admin can send this Job Card to BELM; internal maintenance remains with your own team.";
      setSubmitButtonsText("Send Job Card to BELM");
    } else {
      notice.textContent = `BELM SERVICE PROVIDER ACTIVE: Machine Job Cards are handled by BELM and sent to ${belmBusiness.email || "the BELM Business Email"}. Your other customer portal operations remain under your company.`;
      document.getElementById("portalModeSubtitle").textContent = "BELM Managed Job Card";
      document.getElementById("servicePanelTitle").textContent = "Job Card details";
      document.getElementById("servicePanelIntro").textContent = "Select the work/service required for this machine. Spare requirements stay in the machine spare list and are handled through Procurement.";
      setSubmitButtonsText("Send Job Card to BELM");
    }
    ...
    if (serviceOptions.length) {
      ...
      showAlert("No active Checklist Template matches this machine type. You can submit a general Job Card, or Admin can add the service type and parts first.");
    }
  ...
  document.getElementById("serviceForm").addEventListener("submit", async event => {
    ...
    try {
      const result = await api("/job-cards", {
        method: "POST",
        body: JSON.stringify({
          machineId,
      ...
      showAlert(error.message || "Could not submit Job Card.", true);
    } finally {
      submitButtons().forEach((btn) => btn.classList.remove("success"));
      setSubmitButtonsDisabled(false);
      setSubmitButtonsText(selfServiceMode ? "Send Job Card to BELM" : "Send Job Card to BELM");
      applyBelmJobCardPermission();
  ...
  if (String(tokenPayload().customerRole || "").toLowerCase() === "viewer") {
    ...
    showAlert("Viewer assistants can review Job Card/spare information but cannot save, approve, or submit new records.");
  }
  ...
  async function loadHistory() {
    ...
    try {
      const requests = await api("/job-cards");
      rows.innerHTML = requests.length
        ...
        : '<tr><td colspan="5" class="empty">No Job Cards yet.</td></tr>';
    } catch (error) {
