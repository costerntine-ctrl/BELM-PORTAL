(function () {
  ...
  function clearReceiptInput(){ receiptPhotoData=""; receiptPhotoName=""; document.getElementById("receiptPhoto").value=""; const img=document.getElementById("receiptPreview"); img.removeAttribute("src"); img.classList.remove("hidden"); const name=document.getElementById("receiptFileName"); if(name){name.textContent="";name.classList.add("hidden");} document.getElementById("receiptPreviewWrap").classList.add("hidden"); }
  ...
  document.getElementById("receiptPhoto").addEventListener("change",async e=>{const file=e.target.files?.[0]; if(!file){clearReceiptInput();return;} try{receiptPhotoData=await prepareReceipt(file);receiptPhotoName=file.name;const img=document.getElementById("receiptPreview"),name=document.getElementById("receiptFileName");if(file.type==="application/pdf"){img.removeAttribute("src");img.classList.add("hidden");if(name){name.textContent=`PDF: ${file.name}`;name.classList.remove("hidden");}}else{img.src=receiptPhotoData;img.classList.remove("hidden");if(name){name.textContent=file.name;name.classList.remove("hidden");}}document.getElementById("receiptPreviewWrap").classList.remove("hidden");}catch(err){clearReceiptInput();showAlert(err.message,true);}});
