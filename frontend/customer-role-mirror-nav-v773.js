(function(){
  'use strict';
  if(window.__belmCustomerRoleMirrorV773)return;
  window.__belmCustomerRoleMirrorV773=true;
  ...
  style.textContent=`
    .belm-nav__section-label,.sidebar-nav__section-label{padding:15px 12px 5px;font-size:9px;font-weight:900;letter-spacing:1.35px;text-transform:uppercase;color:#647c98;opacity:.95;pointer-events:none}
    .belm-nav__section-label:first-child,.sidebar-nav__section-label:first-child{padding-top:4px}
    .belm-nav__item small,.nav-item small{display:block;font-size:9px;opacity:.65;font-weight:600;margin-top:2px}
  `;
  ...
  const icons={
    ...
    fuel:'<path d="M5 21V5a2 2 0 012-2h6v18M13 8h4l2 2v8a1.5 1.5 0 003 0v-5l-3-3"/>',
  ...
  const item=(label,href,icon,role)=>({type:'item',label,href,icon,role});
  const section=(label)=>({type:'section',label});

  const configs={
    '/customer-admin-dashboard/':[
      section('Company Control'),
      ...
      section('Workshop & People'),
      item('Workshop Manager','/customer-workshop-manager/','tool','workshop_manager'),
      item('Technicians','/customer-technicians/','tool','technician'),
      item('Machine Operators','/customer-users/#machine-operator-roster','users','operator'),
      item('Job Cards','/breakdown-workflow/?actor=customer','job','workshop_manager'),
      section('Departments'),
      ...
      item('System Settings','/customer-settings-center/','settings')
    ],
    '/customer-workshop-manager/':[
      ...
      item('Machine Condition','/portal/dashboard?view=machines','machine'),
      ...
      item('My Reports','/role-reports/','report'),
      item('Communication','/role-communications/','message'),
    ...
    '/customer-technician-dashboard/':[
      ...
      item('Customer Machines','/portal/dashboard?view=machines','machine'),
      item('Diagnosis & Repair','/technician-job-cards/','tool'),
      item('Spare Requests','/technician-job-cards/','box'),
      item('Testing & Completion','/technician-job-cards/','check'),
      section('Monitoring'),
    ...
    '/customer-procurement-dashboard/':[
      ...
      item('Purchase Records','/customer-procurement-workspace/?view=records','report'),
      item('Pending Proforma','/customer-procurement-workspace/?view=proforma','report'),
    ...
    '/customer-store-dashboard/':[
      ...
      item('Stock Out & Issues','/customer-store-audit/','box'),
    ...
    '/customer-operator-dashboard/':[
      ...
      item('Daily Check / Shift','/operator/','check'),
      item('Operation Log','/operator/','report'),
      item('Fuel Consumption','/operator/','fuel'),
      item('Machine Alerts','/customer-operator-dashboard/#alertList','alert'),
      item('Report Issue','/operator/','message'),
      section('Reporting'),
      item('Communication','/role-communications/','message'),
      item('Operator Reports','/role-reports/','report'),
      item('My Profile','/portal-v2/','users')
    ]
  };
  ...
  const sectionCls=nav.classList.contains('sidebar-nav')?'sidebar-nav__section-label':'belm-nav__section-label';
  let firstItem=true;
  nav.innerHTML=cfg.map(x=>{
    if(x.type==='section')return `<div class="${sectionCls}">${x.label}</div>`;
    const active=firstItem?' is-active':''; firstItem=false;
    return `<a href="${x.href}" class="${cls}${active}"${x.role?` data-customer-role="${x.role}"`:''}>${I(x.icon)}<span>${x.label}</span></a>`;
  }).join('');
})();
