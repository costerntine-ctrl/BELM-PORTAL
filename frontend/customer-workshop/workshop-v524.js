(function(){
  ...
  async function loadCustomer(){
    ...
    try{
      ...
      if($('cwmMachinesLink')){$('cwmMachinesLink').textContent=`${name.toUpperCase()} MACHINES`; $('cwmMachinesLink').href='/portal/dashboard?view=machines&from=cwm'}
      const tech=$('technicianManageLink');if(tech){if(belmOn){tech.textContent='Technicians Locked · BELM ON';tech.removeAttribute('href');tech.setAttribute('aria-disabled','true');tech.classList.add('locked-action')}else{tech.textContent='Manage Technicians';tech.href='/customer-technicians/';tech.removeAttribute('aria-disabled');tech.classList.remove('locked-action')}}
