(function () {
  ...
  function enhanceCustomerAssistants() {
    ...
    for (const button of document.querySelectorAll("button")) {
      ...
      // This entry point now lives inside the "MORE TOOLS" card (renamed
      // "Manage Users", next to Assign Users/Change Password/etc.), so the
      // original header button is always hidden rather than shown. Its
      // text is still normalised to "+ Manage assistants" purely because
      // insertCustomerActivityOverview() uses that exact text to find the
      // header row it should insert the Activity Overview card under.
  ...
  async function insertCustomerActivityOverview() {
    ...
    // Analysis, Manage Users (was "Assign Users" / the header's separate
    // "+ Manage assistants" button — both went to the same /customer-users/
    // page, so they're merged into this one), Change Password, and
    // Management Email used to live inside every single machine card's
    // quick-actions row, but none of them are actually tied to one
    // specific machine — Analysis and the account-level email both pull
    // from the same aggregate /api/customer-portal/analysis data as this
    // Activity Overview card, and Manage Users / Change Password are
    // account settings. Kept to exactly 4 buttons (repeat(4, 1fr) below)
    // so this card never needs to scroll.
    ...
    toolsCard.innerHTML = `
      ...
      <div class="belm-account-tools-actions">
        ...
        <a href="/customer-users/" class="belm-assign-users-button" data-belm-owner-admin-only>Manage Users</a>
