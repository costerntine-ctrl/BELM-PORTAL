(function () {
  ...
  function resumeExistingLoginSession() {
    const currentPath = window.location.pathname;
    if (
      currentPath === "/admin/login"
      && localStorage.getItem("belm_admin_user")
      && tokenIsCurrent(localStorage.getItem("belm_admin_token"))
    ) {
    ...
    if (
      ...
      && tokenIsCurrent(localStorage.getItem("belm_customer_token"))
    ) {
    ...
    if (
      currentPath === "/tech"
      && localStorage.getItem("belm_tech_user")
      && tokenIsCurrent(localStorage.getItem("belm_tech_token"))
    ) {
      return false;
    }
    return false;
  ...
  if (resumeExistingLoginSession()) return;

  function leaveLegacyLoginRoutes() {
    const currentPath = window.location.pathname;
    const query = window.location.search || "";
    if (currentPath === "/admin/login") {
      navigateOnce("/login/?role=admin");
      return true;
    }
    if (currentPath === "/portal/login") {
      navigateOnce(`/login/${query}`);
      return true;
    }
    if (
      currentPath === "/tech"
      ...
      navigateOnce("/login/?role=technician");
      return true;
    }
    return false;
  }

  if (leaveLegacyLoginRoutes()) return;
