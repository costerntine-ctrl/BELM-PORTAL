(()=>{
  ...
  const actionLabel=()=>customerManaged()?'OPEN MACHINE CARD':'OPEN MACHINE CARD';
  ...
  function machineSummary(machine,index,total,openJobs){
    ...
    return `<article class="belm-tech-home-machine status-${s.key}" data-machine-index="${index}" role="button" tabindex="0" aria-label="Open machine card">
  ...
  function render(customer,openJobs){
    ...
    app.querySelector('[data-open-customer-machine]')?.addEventListener('click',openCurrentMachine);
    cards.forEach((card,i)=>{card.addEventListener('click',()=>{show(i);openCurrentMachine()});card.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();show(i);openCurrentMachine()}})});
