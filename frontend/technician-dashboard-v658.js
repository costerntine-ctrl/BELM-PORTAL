(()=>{
  ...
  async function openReports(machine){
    ...
    try{
      ...
      body.innerHTML=`<div class="machine-report-types">${types.map(([key,title,rows,count])=>`<article class="machine-report-type" data-period-card="${key}"><div class="machine-report-label"><b>${esc(title)}</b><small>${Number(count)} report(s)</small></div><div class="machine-report-period"><label><span>From</span><input type="date" data-period-from></label><label><span>To</span><input type="date" data-period-to></label></div><div class="machine-report-actions"><button type="button" data-report-view="${key}">View Report</button><button type="button" data-period-pdf="${key}">Period PDF</button><button type="button" data-period-csv="${key}">Period CSV</button></div></article>`).join('')}</div>`;
      const periodParams=button=>{const card=button.closest('[data-period-card]'),from=card?.querySelector('[data-period-from]')?.value||'',to=card?.querySelector('[data-period-to]')?.value||'';if(from&&to&&from>to)throw new Error('From date cannot be after To date.');const p=new URLSearchParams({category:button.dataset.reportView||button.dataset.periodPdf||button.dataset.periodCsv||'',machineId:String(machine.id)});if(from)p.set('from',from);if(to)p.set('to',to);return p};
      body.querySelectorAll('[data-report-view]').forEach(button=>button.onclick=()=>{try{const p=periodParams(button);p.set('v','666');location.href=`/tech-report/?${p.toString()}`}catch(e){alert(e.message)}});
      body.querySelectorAll('[data-period-pdf]').forEach(button=>button.onclick=()=>{try{const p=periodParams(button);reportFile(`/api/checklist-reports/technician-general/pdf?${p}`,`BELM-${machine.model||'Machine'}-${button.dataset.periodPdf}-period.pdf`,button).catch(e=>alert(e.message))}catch(e){alert(e.message)}});
      body.querySelectorAll('[data-period-csv]').forEach(button=>button.onclick=()=>{try{const p=periodParams(button);reportFile(`/api/checklist-reports/technician-general/csv?${p}`,`BELM-${machine.model||'Machine'}-${button.dataset.periodCsv}-period.csv`,button).catch(e=>alert(e.message))}catch(e){alert(e.message)}});
  ...
  function card(machine,nativeButton){
    ...
    el.innerHTML=`
      ...
      <section class="alerts"><div class="message-display"><small data-message-title>LIVE MACHINE MESSAGE</small><strong data-message-text>Loading machine message…</strong><p data-message-meta>Please wait</p><div class="message-dots" aria-hidden="true"></div></div></section>
    ...
    const rotatingMessages=[
      {title:'OPERATOR MESSAGE',text:op.text,meta:op.meta,type:'operator'},
      {title:'MACHINE ALERT',text:reasons(machine,c),meta:c.status==='RED'?'Immediate correction required':c.status==='YELLOW'?'Inspection or maintenance required':'No active critical alert',type:'alert'},
      {title:'SERVICE STATUS',text:`Service range: ${machine.serviceRange||machine.service_range||'checking…'}`,meta:`Activity: ${opLabels[status]||status}`,type:'service'}
    ];
    let messageIndex=Math.floor(Math.random()*rotatingMessages.length);
    const titleNode=el.querySelector('[data-message-title]'),textNode=el.querySelector('[data-message-text]'),metaNode=el.querySelector('[data-message-meta]'),dotsNode=el.querySelector('.message-dots');
    const showMessage=()=>{const message=rotatingMessages[messageIndex];titleNode.textContent=message.title;textNode.textContent=message.text;metaNode.textContent=message.meta;el.querySelector('.message-display').dataset.messageType=message.type;dotsNode.innerHTML=rotatingMessages.map((_,index)=>`<i class="${index===messageIndex?'active':''}"></i>`).join('')};
    showMessage();
    setInterval(()=>{let next=messageIndex;while(next===messageIndex)next=Math.floor(Math.random()*rotatingMessages.length);messageIndex=next;showMessage()},5000);
  ...
  const style=document.createElement('style');style.textContent=`
    ...
    .belm-tech658-card .alerts{margin-top:18px;padding:15px;border:1px solid #0b78bd;border-radius:22px;background:#021329}.belm-tech658-card .message-display{min-height:170px;padding:22px 18px;border:3px solid var(--status);border-radius:20px;box-shadow:0 0 16px rgba(var(--rgb),.48);display:flex;flex-direction:column;justify-content:center}.belm-tech658-card .alerts small{color:#18a9f4}.belm-tech658-card .alerts strong{display:block;color:var(--status);font-size:17px;line-height:1.35;margin-top:10px}.belm-tech658-card .alerts p{margin:10px 0 0;color:#c3cedb;font-size:12px}.belm-tech658-card .message-display[data-message-type="service"] strong{color:#ffc21c}.belm-tech658-card .message-dots{display:flex;justify-content:center;gap:7px;margin-top:18px}.belm-tech658-card .message-dots i{width:7px;height:7px;border-radius:50%;background:#607892}.belm-tech658-card .message-dots i.active{background:#ffc21c;box-shadow:0 0 8px rgba(255,194,28,.7)}
