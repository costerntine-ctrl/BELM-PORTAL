(()=>{
  ...
  const download=async(url,name,button)=>{const old=button.textContent;button.disabled=true;button.textContent='Preparing…';try{const r=await fetch(url,{cache:'no-store',headers:{Authorization:`Bearer ${token()}`}});if(!r.ok){const e=await r.json().catch(()=>({}));throw new Error(e.error||'Could not download report.')}const blob=await r.blob(),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}finally{button.disabled=false;button.textContent=old}};
  ...
  async function openReports(machine){
    ...
    try{
      ...
      const rowHtml=(key,rows)=>rows.length?`<div class="report-list">${rows.map(r=>{if(key==='checklists')return `<article><b>${esc(r.templateName||'Checklist Report')}</b><span>${esc(r.status||'GREEN')} · Hour meter: ${esc(r.hourMeterReading??'—')}</span><small>${esc(r.createdAt||'')} · ${esc(r.filledBy||'Not recorded')}</small></article>`;if(key==='operator')return `<article><b>${esc(r.operatorName||'Operator')}</b><span>${esc(r.status||'OPEN')}</span><small>${esc(r.message||'No message')} · ${esc(r.createdAt||'')}</small></article>`;if(key==='fuel')return `<article><b>${esc(r.litres||0)} Litres · TZS ${Number(r.cost||0).toLocaleString('en-TZ')}</b><span>${esc(r.description||'Fuel')}</span><small>${esc(r.date||r.createdAt||'')} · By ${esc(r.loggedBy||'Not recorded')}</small></article>`;if(key==='job-cards')return `<article><b>${esc(r.jobCardNo||'Job Card')} · ${esc(r.title||'Maintenance')}</b><span>${esc(r.status||'OPEN')}</span><small>${esc(r.faultDescription||r.createdAt||'')}</small></article>`;return `<article><b>${esc(r.serviceType||'Maintenance')}</b><span>Hour meter: ${esc(r.hourMeterReading??'—')}</span><small>${esc(r.date||'')} · ${esc(r.recordedBy||'Not recorded')}</small></article>`}).join('')}</div>`:'<p class="empty-report">No report records found.</p>';
      const render=(active='checklists')=>{body.innerHTML=`<div class="machine-report-types">${types.map(([key,title,rows,count])=>`<article class="machine-report-type ${key===active?'active':''}"><div><b>${esc(title)}</b><small>${Number(count)} report(s)</small></div><div class="machine-report-actions"><button type="button" data-report-view="${key}">View Report</button><button type="button" data-report-download="${key}">Download</button></div></article>`).join('')}</div><section class="machine-report-view"><h3>${esc(types.find(t=>t[0]===active)?.[1]||'Report')}</h3>${rowHtml(active,types.find(t=>t[0]===active)?.[2]||[])}</section>`;body.querySelectorAll('[data-report-view]').forEach(b=>b.onclick=()=>render(b.dataset.reportView));body.querySelectorAll('[data-report-download]').forEach(b=>b.onclick=()=>download(`/api/checklist-reports/technician-general/pdf?category=${encodeURIComponent(b.dataset.reportDownload)}&machineId=${encodeURIComponent(machine.id)}`,`BELM-${machine.model||'Machine'}-${b.dataset.reportDownload}.pdf`,b).catch(e=>alert(e.message)))};
      render();
