(()=>{
  ...
  async function openReports(machine){
    const d=modal(`${machine.model||'Machine'} — Reports`,'<p class="loading">Loading completed inspections…</p>');
    try{
      const rows=await api(`/api/checklist-reports/machine/${encodeURIComponent(machine.id)}`);
      const list=Array.isArray(rows)?rows:[];
      d.querySelector('.belm-tech658-modal-body').innerHTML=list.length?`<div class="report-list">${list.map(r=>`<article><b>${esc(r.checklistNo||r.checklist_no||`Checklist #${r.id||''}`)}</b><span>${esc(r.overallStatus||r.overall_status||'Recorded')}</span><small>${esc(r.createdAt||r.created_at||'')}</small></article>`).join('')}</div>`:'<p>No completed inspection reports yet.</p>';
