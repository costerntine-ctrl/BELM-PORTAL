(()=>{
  ...
  async function openReports(machine){
    ...
    try{
      ...
      const render=(active='checklists')=>{const selected=types.find(t=>t[0]===active)||types[0],base=`/api/checklist-reports/technician-general`,query=`category=${encodeURIComponent(active)}&machineId=${encodeURIComponent(machine.id)}`,file=`BELM-${machine.model||'Machine'}-${active}`;body.innerHTML=`<div class="machine-report-types">${types.map(([key,title,rows,count])=>`<article class="machine-report-type ${key===active?'active':''}"><div><b>${esc(title)}</b><small>${Number(count)} report(s)</small></div><div class="machine-report-actions"><button type="button" data-report-view="${key}">View Report</button></div></article>`).join('')}</div><section class="machine-report-view"><div class="opened-report-head"><h3>${esc(selected[1])}</h3><div><button type="button" data-opened-print>Print</button><button type="button" data-opened-pdf>Download PDF</button><button type="button" data-opened-csv>CSV File</button></div></div>${rowHtml(active,selected[2])}</section>`;body.querySelectorAll('[data-report-view]').forEach(b=>b.onclick=()=>render(b.dataset.reportView));body.querySelector('[data-opened-print]').onclick=e=>reportFile(`${base}/pdf?${query}`,`${file}.pdf`,e.currentTarget,true).catch(x=>alert(x.message));body.querySelector('[data-opened-pdf]').onclick=e=>reportFile(`${base}/pdf?${query}`,`${file}.pdf`,e.currentTarget).catch(x=>alert(x.message));body.querySelector('[data-opened-csv]').onclick=e=>reportFile(`${base}/csv?${query}`,`${file}.csv`,e.currentTarget).catch(x=>alert(x.message))};
      render();
