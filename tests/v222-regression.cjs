...
{
  const m = /workflow\.css\?v=(\d+)-/.exec(html);
  check('Breakdown CSS cache key bumped', !!m && Number(m[1]) >= 222);
}
...
check('PWA cache bumped',/CACHE='belm-app-v(\d+)-/.exec(sw) && Number(/CACHE='belm-app-v(\d+)-/.exec(sw)[1]) >= 222);
