...
test('deploy schema does not rewrite unassigned card lifecycle',!schema.includes("SET status='RECEIVED'")&&schema.includes('V350 DATA SAFETY'));
test('deploy schema does not rewrite assigned card lifecycle',!schema.includes("SET status='ASSIGNED'")&&schema.includes('V350 DATA SAFETY'));
...
test('migration protects business IDs instead of normalizing live rows',migrate.includes('belm_predeploy_ids')&&migrate.includes('protected records preserved'));
