...
t('JC Number dispatch field remains available',html.includes('JC Number')&&html.includes('Fill the received JC Number to assign technician'));
t('received selection auto fills JC code',js.includes('syncDispatchJcNumberFromSelection')&&js.includes('jc-auto-detected'));
t('manual JC code can resolve received Job Card',js.includes('resolveDispatchJobCardNumber')&&(eng.includes('JC Number / Proforma Code was not found')||eng.includes('JC Number was not found')));
...
t('Job Card traceability survives Proforma numbering', (pro.includes("$invoiceNo = $sourceJobCardId !== '' && $sourceJobCardNo !== ''")&&pro.includes('? $sourceJobCardNo')) || (pro.includes("belm_next_commercial_number('PI')")&&pro.includes('source_job_card_id')));
