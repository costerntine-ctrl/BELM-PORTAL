...
const checks = [
  ...
  ['working portal link is shown in MY C', html.includes('Customer identity and portal access') && app.includes('new URL("/login", window.location.origin).href')],
  ['copy link works in MY C', app.includes('data-copy-customer-link') && app.includes('navigator.clipboard.writeText')],
  ['open customer login works in MY C', app.includes('class="open-login"') && app.includes('target="_blank"')],
